
public class Constant {

	//public final static String BASE_PATH = "cyclients\\linearhash\\";
	
	//public final static String WORKSPACE_PATH =  BASE_PATH + "workspace\\";
		
	public final static String DISK_PATH = "device_0.RAF";
	
	public final static String DISK_ALLOC =  DISK_PATH + "_alloc";
	
	public final static String DISK_DEALLOC = DISK_PATH + "_dealloc";
	
	
	//public final static String LH_SERIAL_PATH = WORKSPACE_PATH + "LH_serialized.ser";
	public final static String LH_SERIAL_PATH = "LH_serialized.ser";
	
	//public final static String TUPLE_CONFIG_XML = WORKSPACE_PATH + "tuple_config.xml";
	public final static String TUPLE_CONFIG_XML = "tuple_config.xml";
	
	public final static Integer M_INIT = 3;
	
	public final static Integer SP_INIT = 0;
	
	public final static int ADDRESS_SIZE = 4;
	
	public final static double LAMBDA_AVG = 1.5;
	
	public final static int M_INC = 2;
	
	
	public Constant() {
		// TODO Auto-generated constructor stub
	}
	
}