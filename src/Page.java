...
public class Page {
	...
	public enum DELETE_STATUS{SUCCESS}
 ...
 */
	...
	public static DELETE_STATUS deleteTuple(LinearHash lhash, byte[] firstPageBuf, byte [] tuple) throws IOException {
		
		return _deleteTuple(lhash, firstPageBuf, tuple);
		
	}
	
	private static DELETE_STATUS _deleteTuple(LinearHash lhash, byte[] firstPageBuf, byte[] tuple)
	{
		int[] page_offset = _getIndex(lhash.getTableName(), firstPageBuf, Tuple.readKey(lhash.getTableName(), tuple));
		TupleIterator iter = Page.getTupleIterator(lhash.getTableName(), tuple);
		iter.setIteratorCurPos(page_offset[1]);
		byte[] curPageBuf = new byte[Page.PAGE_SIZE];
		LinearHash.getDisk().readPage(curPageBuf, page_offset[0]);
		iter.setIteratorPageBuf(curPageBuf);
		if(iter.curPos == Page.getNoOfTuples(iter.getIteratorPageBuf()) && -1 == Page.getNextPage(iter.getIteratorPageBuf()))
		{
			DELETE_STATUS deleted = _delTuple(lhash.getTableName(), iter.getIteratorPageBuf(), tuple);
			LinearHash.getDisk().writePage(iter.getIteratorPageBuf(), getPageNo(iter.getIteratorPageBuf()));
			
			byte [] lastPageBuf = getLastPage(iter.getIteratorPageBuf(), new StringBuffer());
			
			if (DELETE_STATUS.SUCCESS == deleted) {
				LinearHash.getDisk().writePage(lastPageBuf, getPageNo(lastPageBuf));
				return deleted;
			}
	...
	private static int[] _getIndex(String tableName, byte[] firstPage, byte[] key)
	{
		TupleIterator iter = Page.getTupleIterator(tableName, firstPage);
		byte[] tuple = new byte[Tuple.TupleSize(tableName)];
		tuple = iter.getNext();
	...
	private static DELETE_STATUS _delTuple(String tableName, byte[] pageBuf, byte[] tuple)
	{
		TupleIterator iter = Page.getTupleIterator(tableName, pageBuf);
		while(null != iter.getNext())
		{
		}
		int [] tuple_start = pageOffset.get(PAGE_ITEMS.TUPLE_START);
	...
	//	if (-1 == next_page && Page.MaxTuples(tableName) <= no_of_tuples) return DELETE_STATUS.PAGE_FULL;
		int tuple_len = tuple.length;
		for(int i = 0; i< tuple_len; i++ ){
			tuple[i] = 0;
		}
		System.arraycopy(tuple, 0, pageBuf, indx, tuple.length);
	//	setTuple(tableName, pageBuf, indx, tuple);
		no_of_tuples += 1;
		setNoOfTuples(pageBuf, no_of_tuples);
